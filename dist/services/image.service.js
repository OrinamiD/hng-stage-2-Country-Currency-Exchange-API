...
export const generateSummaryImage = async () => {
    const client = await pool.connect();
    try {
        const topResult = await client.query("SELECT * FROM countries ORDER BY estimated_gdp DESC NULLS LAST LIMIT 5");
        const topCountries = topResult.rows;
        const countResult = await client.query("SELECT COUNT(*) AS total FROM countries");
        const totalCountries = parseInt(countResult.rows[0].total, 10);
        const metaResult = await client.query("SELECT value FROM metadata WHERE key=$1", ["last_refreshed_at"]);
