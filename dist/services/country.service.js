...
export const refreshCountry = async () => {
    ...
    try {
        ...
        for (const c of countries) {
            ...
            const exchangeRate = currency && rates[currency] !== undefined ? rates[currency] : null;
            const randomMultiplier = Math.floor(Math.random() * (2000 - 1000 + 1)) + 1000;
            const estimated_gdp = c.population && exchangeRate !== null
...
export const getAllCountry = async (filters) => {
...
export const getCountryName = async (name) => {
    const result = await pool.query("SELECT * FROM countries WHERE LOWER(name)=LOWER($1)", [name]);
    if (result.rows.length === 0)
        throw new Error("Country not found");
    return result.rows[0];
};
export const deleteOneCountry = async (name) => {
    const result = await pool.query("DELETE FROM countries WHERE LOWER(name)=LOWER($1)", [name]);
    if (result.rowCount === 0)
        throw new Error("Country not found");
    return { message: `${name} deleted successfully` };
};
export const fetchStatus = async () => {
    const countResult = await pool.query("SELECT COUNT(*) AS total_countries FROM countries");
    const metaResult = await pool.query("SELECT value FROM metadata WHERE key=$1", ["last_refreshed_at"]);
    return {
